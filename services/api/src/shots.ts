...
export interface ShotStore {
  find(id: string): Promise<SavedShot | undefined>;
  save(shot: Omit<SavedShot, "created_at">): Promise<SavedShot>;
  list(limit: number, offset: number): Promise<SavedShot[]>;
}
...
export function createShotsRouter(store: ShotStore, predict = getXGPrediction) {
  ...
  router.post("/shots", async (req, res) => {
    ...
    try {
      ...
      const saved = await store.save({ id, x, y, ...prediction, distance_yards: geometry.distance_yards, angle_degrees: geometry.angle_degrees });
