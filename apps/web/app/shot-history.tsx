...
import { listShots, saveShot, shotExportUrl, type SavedShot, type ShotContext } from "@/lib/api";

export default function ShotHistory({ shot, canSave, onSelect }: {
  shot: { x: number; y: number; context: ShotContext };
  canSave: boolean;
  onSelect: (shot: { x: number; y: number; context: ShotContext | null }) => void;
}) {
  ...
  const pending = useRef<{ id: string; x: number; y: number; context: ShotContext } | null>(null);
  const saveLock = useRef(false);
  const key = (value: {x: number; y: number; context: ShotContext | null}) => JSON.stringify([value.x, value.y, value.context?.body_part, value.context?.technique, value.context?.shot_type, value.context?.play_pattern]);
  const positionKey = key(shot);
  ...
  async function save() {
    ...
    setMessage("");
    setSaveFailed(false);
    if (!pending.current || key(pending.current) !== positionKey) pending.current = { id: crypto.randomUUID(), x: shot.x, y: shot.y, context: shot.context };
    try {
      const result = await saveShot(pending.current);
      setSavedPosition(key(result));
    ...
    <section className="history-section" aria-labelledby="history-title">
      ...
      <div className="history-card" aria-busy={loading}>
        ...
        {loading ? <div className="history-placeholder" role="status"><RefreshCw className="loading-ring" size={22} /><p>Loading your saved chances…</p></div> : error ? <div className="history-placeholder"><p role="alert">{error}</p><button className="button secondary" onClick={() => refresh(offset)}>Try again</button></div> : rows.length === 0 ? <div className="history-placeholder"><span className="empty-icon"><Bookmark size={24} /></span><h3>A collection starts with one chance.</h3><p>Try a shot above, then save it to build your collection.</p></div> : <>
          <div className="desktop-history"><table><caption className="sr-only">Saved chances, newest first. Distances are in yards, angles in degrees.</caption><thead><tr>{["Saved", "Position (x, y)", "Distance", "Angle", "Expected goals", ""].map((heading) => <th key={heading} scope="col">{heading || <span className="sr-only">Action</span>}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} className={row.x === shot.x && row.y === shot.y ? "active-row" : ""}><td><time dateTime={row.created_at} title={new Date(row.created_at).toLocaleString()}>{date(row.created_at)}</time><small className="saved-model">{modelLabel(row.model_id)}</small><small className="saved-model">{row.context ? Object.values(row.context).join(" · ") : "Context not recorded"}</small></td><td className="numeric muted">{row.x.toFixed(1)}, {row.y.toFixed(1)}</td><td className="numeric">{row.distance_yards.toFixed(1)} <span className="muted">yd</span></td><td className="numeric">{row.angle_degrees.toFixed(1)}°</td><td><span className="table-xg">{(row.xg_probability * 100).toFixed(1)}%<span className="mini-track"><i style={{ width: `${row.xg_probability * 100}%` }} /></span></span></td><td><button className="text-button" aria-label={`Revisit shot at ${row.x.toFixed(1)}, ${row.y.toFixed(1)}`} onClick={() => onSelect({ x: row.x, y: row.y, context: row.context })}>Revisit <ArrowUpRight size={15} /></button></td></tr>)}</tbody></table></div>
          <ul className="mobile-history">{rows.map((row) => <li key={row.id}><div><span><time dateTime={row.created_at}>{date(row.created_at)}</time><small className="saved-model">{modelLabel(row.model_id)}</small><small className="saved-model">{row.context ? Object.values(row.context).join(" · ") : "Context not recorded"}</small></span><strong>{(row.xg_probability * 100).toFixed(1)}% <small>xG</small></strong></div><div><span>{row.distance_yards.toFixed(1)} yd · {row.angle_degrees.toFixed(1)}°</span><button className="text-button" aria-label={`Revisit shot at ${row.x.toFixed(1)}, ${row.y.toFixed(1)}`} onClick={() => onSelect({ x: row.x, y: row.y, context: row.context })}>Revisit <ArrowUpRight size={15} /></button></div></li>)}</ul>
        </>}
