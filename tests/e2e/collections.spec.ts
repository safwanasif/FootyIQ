...
test("save, compare, revisit, download, reload and isolate two browsers", async ({ page, browser }) => {
  ...
  await page.getByLabel("Body part", { exact: true }).selectOption("Left Foot");
  ...
  expect(initial.shots[0].context.body_part).toBe("Left Foot");
  ...
  await expect(page.getByLabel("Body part", { exact: true })).toHaveValue("Left Foot");
  ...
  expect(csv).toContain("Left Foot");
